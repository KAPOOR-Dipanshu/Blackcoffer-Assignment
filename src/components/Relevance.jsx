...
const Relevance = ({ data }) => {
    ...
    useEffect(() => {
        ...
        // Extracting distinct relevance values
        const distinctIntensities = Array.from(new Set(data.map(d => d.relevance)));

        // Creating a histogram to count occurrences of each relevance
        const relevanceHistogram = d3.histogram()
            .value(d => d.relevance)
            .domain(d3.extent(data, d => d.relevance))
            .thresholds(distinctIntensities.length)(data);

        // Filtering out bars with count 0
        const filteredHistogram = relevanceHistogram.filter((d, i) => d.length !== 0 && i !== 0);
